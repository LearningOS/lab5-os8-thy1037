...
pub fn sys_mutex_lock(mutex_id: usize) -> isize {
    ...
    if process_inner.deadlock_detect == true && process_inner.available[mutex_id] == 0 {
        drop(process_inner);
        -0xdead
    } else {
...
pub fn sys_semaphore_down(sem_id: usize) -> isize {
    ...
    let wait_len = sem.inner.exclusive_access().wait_queue.len();

    if process_inner.deadlock_detect == true && process_inner.available[sem_id] - 1 <= wait_len {
        drop(process_inner);
        -0xdead
    } else {
        process_inner.available[sem_id] -= 1;
